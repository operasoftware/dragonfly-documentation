...
CHRIS - OVERALL COMMENT. THIS IS PRETTY GOOD OVERALL, ALTHOUGH I DO WONDER IF IT WOULD BE WAY MORE USEFUL IF WE HAVE SOME ACTUAL WALKTHROUGH EXAMPLES TO BACK UP THE EXPLANATION OF THE FEATURES. TO SEE THEM IN CONTEXT, AS IT WERE. CAN WE WRITE SOME DEV.O ARTICLES TO BACK IT UP LATER ON PERHAPS?

// TODO: Check terms. Remove DataTips as they probably wont make it. Add screenshots.

CHRIS - DO WE WANT AN "INTRODUCTION" HEADING UNDER THE INTRODUCTION? ADDING ONE, JUST IN CASE.
...
Note: Opera Dragonfly also features a Console HUD (HUD ISN'T VERY SUITABLE FOR A TECH MANUAL - CHANGE TO "INTERFACE"?), which is fully integrated with the debugger. (CHRIS - MOVED TO A SEPARATE NOTE PARAGRAPH, AS IT WAS MAKING THE FLOW OF THE ABOVE PARAGRAPH SOUND A BIT ODD. DO WE WANT TO ADD A LINK TO THE CHAPTER THAT COVERS THIS FEATURE?)

### Source view ###

####Selecting a script

Any linked, inline (CHRIS - EXTERNAL, INTERNAL, INLINE? LINKED AND INLINE DON'T REALLY COVER SCRIPTS INSIDE THE SCRIPT TAG?) and evaled scripts can be selected from the document selector (CHRIS - THE DROP DOWN MENU?) in the Scripts toolbar[1]. These are organized by parent runtime, so that if there are multiple documents–such as iframes or linked SVG files–the scripts will be listed under the document they are defined in. Injected scripts such as Browser.js and User.js (CHRIS - CODE FONT FOR FILE NAME, OR NOT? WE DO THIS ON DEV.O) are also listed under their own heading.

CHRIS - MENTION THAT YOU MIGHT NEED TO CLICK THE REFRESH BUTTON TO RELOAD THE SCRIPTS, SO THAT THEY WILL BE AVAILABLE UNDER THE DROPDOWN TO DEBUG? THIS CON FUSED ME FOR A FEW SECONDS.


#### The Source panel

Once a script is selected, the source is shown in the main panel. From this panel breakpoints can be set to control the flow of the program by clicking on the line numbers in the gutter. Dots appear to show you where breakpoints have been set, and you can toggle them off again by clicking on them. Breakpoints are covered in more detail in the "Breakpoints" section below (CHRIS - IS THIS OK FOR A FORWARD REFERENCE?).

#### Search

Opera Dragonfly supports a number of advanced methods to search inside scripts. Clicking the search button[2] brings up a search toolbar. From here it is possible to enter a search term, and navigate between the results with either the forward and backwards buttons or the <kbd>Enter</kbd> and <kbd>Shift</kbd>+<kbd>Enter</kbd> keys. (CHRIS - WHEN I ENTER A SEARCH TERM AND TRY TO SEARCH THROUGH THEM USING THESE CONTROLS, THE INSTANCE NUMBER (EG 2 OUT OF 23) GOES UP AND DOWN, BUT THE ACTUAL SCRIPT WINDOW DOESN'T SEEM TO MOVE TO SHOW THE DIFFERENT INSTANCES...)
...
Pressing <kbd>Ctrl</kbd>+<kbd>M</kbd> (<kbd>⌘</kbd>+<kbd>L</kbd>) when the source panel is active will open the Go to line window. Entering a number will scroll to the specified line. //double check M is the correct shortcut on Windows. (CHRIS - THIS CURRENTLY DOESN'T SEEM TO WORK - THE SHORTCUT WORKS, BUT ENTERING A LINE NUMBER DOESN'T MAKE THAT LINE COME UP.)
...
CHRIS - THIS MATERIAL IS GOOD, BUT SHOULDN'T IT COME UNDER THE BREAKPOINTS SECTION, RATHER THAN BEING IN IT'S OWN SECTION? THE TWO SUBJECTS ARE VERY CLOSELY LINKED. AND AT THIS POINT, YOU'RE TALKING ABOUT BREAKPOINTS BEFORE THEY'VE REALLY BEEN DEFINED.
...
(CHRIS - SOMETHING NOT VERY OBVIOUS IS - WHEN YOU'VE SET BREAKPOINTS, HOW DO YOU THEN GET THE CODE TO EXECUTE SO YOU CAN STEP THROUGH ALL THE BREAKPOINTS, ETC? I'M EXPECTING A BIG OBVIOUS GO BUTTON. DO YOU HAVE TO RELOAD THE PAGE TO GET IT TO RUN THROUGH THE SCRIPT? IF SO, HOW?)
...
A regular line or event breakpoint can be changed to a conditional breakpoint by selecting <q>Add condition<q> from the context menu of the appropriate breakpoint in the Breakpoints panel, and entering a valid expression in the <q>condition</q> field (THERE SEEMS TO BE SOME INCONSISTENCY HERE - SOME OF THE LABELS FROM THE INTERFACE ARE IN QUOTES, AND SOME AREN'T - WHICH STYLE DO YOU WANT TO STICK TO?). The expression must return a boolean value. The breakpoint symbol in the source view gutter will change to indicate it has a condition attached (a plus sign will appear in the relevant breakpoint circle).

To get rid of a condition, remove the text inside the condition field and press Enter/Return, and it will disappear.

For line breakpoints, it is also possible to right click on the breakpoint circle symbol in the source view gutter and select <q>Add condition</q>.

Examples of conditions include testing if a variable equals a specific value (<samp>foo == 10</samp>), a variable is greater than another variable (<samp>foo > bar</samp>, a function returns <q>true</q> (<samp>foo.bar(baz) === true</samp>), or when an event breakpoint fires, if it is a specific element (<samp>event.target.id == "foo"</samp>). (CHRIS - THIS WOULD BE MUCH BETTER PRESENTED AND EASIER TO DIGEST AS A TABLE.)

#### Deleting and disabling breakpoints ####

An individual breakpoint can be disabled by clicking on the breakpoint icon in the source view gutter (for line breakpoints) or unchecking the check box next to the name of the breakpoint in the breakpoints panel. The icon in the gutter will change to an empty circle to show it is disabled.
...
(IT SEEMS WEIRD TO ME THAT THE DISABLE ALL BUTTON CAN'T BE PRESSED AGAIN TO TOGGLE THE BREAKPOINTS ALL AS ENABLED. SEEMS TO GO AGAINST EXPECTED BEHAVIOUR)
...
As the number of properties can start to get unwieldy, it is possible to hide variables if they still have their default values—null or empty strings—by unclicking the relevant button on the Inspection toolbar. It is also possible to hide enumerable properties.

You can also filter the visible properties by a search term using the Filter field in the toolbar.

<!--
#### Data Tips ####
@@@@not implemented yet. Check back later to be sure and describe better. Will probably not make it so can comment out.

When execution is paused at a breakpoint, hovering over variables in the the source will show tool tips, which show the current value at that point in time. If the variable is an object or array, it is possible to drill down into it to see all the enclosed values.
-->
...
(CHRIS - AGAIN, WOULD BE EASIER TO DIGEST THIS IF IT WERE PRESENTED AS A TABLE.)
